...
namespace iroha {
  namespace ametsuchi {
    ...
    StorageImpl::createMutableStorage() {
      ...
      nonstd::optional<shared_model::crypto::Hash> top_hash;

      blocks_->getTopBlocks(1)
          .subscribe_on(rxcpp::observe_on_new_thread())
          .as_blocking()
          .subscribe([&top_hash](auto block) { top_hash = shared_model::crypto::Hash(block.hash.to_string()); });

      return expected::makeValue<std::unique_ptr<MutableStorage>>(
          std::make_unique<MutableStorageImpl>(
              top_hash.value_or(shared_model::crypto::Hash("")),
              std::move(postgres_connection),
              std::move(wsv_transaction),
              std::move(command_executors.value())));
    }
